...
export class RequestService {
  ...
  getListHosp(hcode, mcode, scode, query = '', limit, offset) {
    const url = `${this.url}/v2/admin/hospital?hosptype_code=${hcode}&ministry_code=${mcode}&
    sub_ministry_code=${scode}&query=${query}&limit=${limit}&offset=${offset}`;
    return this.http.get(url).toPromise();
  }

  getListHospTotal(hcode, mcode, scode, query) {
    const url = `${this.url}/v2/admin/supplies-min-max/hosp-balance?hosptype_code=${hcode}&ministry_code=${mcode}&sub_ministry_code${scode}&query=${query}`;
    return this.http.get(url).toPromise();
  }

  getSupplies(hospcode) {
    const url = `${this.url}/v2/admin/supplies-min-max/balance?hospcode=${hospcode}`;
    return this.http.get(url).toPromise();
  }
