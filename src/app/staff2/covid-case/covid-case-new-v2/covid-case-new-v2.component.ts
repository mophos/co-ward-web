...
export class CovidCaseNewV2Component implements OnInit {
  ...
  async next() {
    try {
      ...
      if (rs.ok) {
        if (rs.case === 'NEW') {
          this.router.navigate(['/staff2/covid-case-new', { province: this.province, caseStatus: this.caseType, peopleType: this.peopleType, cid: this.cid, passport: this.passpost, typeRegister: this.typeRegister }]);
